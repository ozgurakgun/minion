...
struct GCC : public AbstractConstraint
{
    ...
    virtual void full_propagate()
    {
        ...
        {
            capacity_array[i].setMin(0, label());
            capacity_array[i].setMax(numvars, label());
        }
        do_gcc_prop();
    }
  ...
  virtual void special_check()
  {
    ...
    void do_gcc_prop()
    {
        ...
        for(int i=0; i<numvals; i++)
        {
            ...
            capacity_array[i].setMin(mincap, label());
            capacity_array[i].setMax(maxcap, label());
        }
                            ...
                            for(int i=0; i<var_indices.size(); i++)
                            {
                                ...
                                if(!varinlocalmatching.in(curvar))
                                {
                                    ...
                                    if(varvalmatching[curvar]!=copynode+dom_min-numvars)
                                    {
                                        ...
                                        var_array[curvar].removeFromDomain(copynode+dom_min-numvars, label());
                                    }
                                }
                            }
                        }
