...
struct DifferenceConstraint : public AbstractConstraint
{
  ...
  void remove_range(DomainInt low, DomainInt high, Var& v)
  {
    ...
    if(!v.isBound())
    {
      for(DomainInt i = low + 1; i < high; ++i)
        v.removeFromDomain(i, label());
    } 
    else
    {
      if(v.getMax() < high)
        v.setMax(low + 1, label());
      
      if(v.getMin() > low)
        v.setMin(high - 1, label());
    }
  }
  ...
  PROPAGATE_FUNCTION(int, DomainDelta)
  {
    ...
    var3.setMax(max(var2_max, var1_max) - min(var1_min, var2_min), label());

    var1.setMin(var2.getMin() - var3.getMax(), label());
    var2.setMin(var1.getMin() - var3.getMax(), label());
    var1.setMax(var2.getMax() + var3.getMax(), label());
    P(var2.getMax());
    var2.setMax(var1.getMax() + var3.getMax(), label());
    P(var2.getMax());
        
    if(var1_max < var2_min)
    {
      var3.setMin(var2_min - var1_max, label());
      var2.setMin(var1.getMin() + var3.getMin(), label());
      var1.setMax(var2.getMax() - var3.getMin(), label());
    }

    if(var2_max < var1_min)
    {
      var3.setMin(var1_min - var2_max, label());
      var1.setMin(var2.getMin() + var3.getMin(), label());
    P(var2.getMax());
          var2.setMax(var1.getMax() - var3.getMin(), label());
              P(var2.getMax());
    }
