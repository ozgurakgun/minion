...
public:
  ...
  SearchState(StateObj* _stateObj) : stateObj(_stateObj), nodes(0), optimise_var(NULL), 
    ...
    solutions(0), dynamic_triggers_used(false), finished(false), 
    failed(false), tupleListContainer(NULL), is_locked(false),
    alarm_trigger(false), ctrl_c_pressed(false), failure(NULL)
  {}
